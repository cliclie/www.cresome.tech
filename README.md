...
- **配置**: 全ページ共通の背景レイヤー。`position: fixed; z-index: 0; pointer-events: none`（`src/index.css` 参照）。`.wave-bg` は `inset: 0`（フルビューポート）、`.sw-bg` は `inset: 64px`（画面内側 64px にインセット）。`.main`(z-1) / サイドバー・トップバー(z-40) / ドロワー(z-50) より奥にあるため既存 UI への変更は不要。サイドバー背景は透明のためその内側にも見える。カード等は白背景のため本文の可読性は維持。
- **品質・性能**:

...
- **モード切替（1行目）**: 自動（既定）/ 手動。自動モードでは始点・向きのボタンが無効化され、ルート完了（fadeOut → 始点スナップ、`onRouteLoop`）のたびに始点＋向きをランダムに再抽選する。抽選は「直近 10 回の選択履歴」（localStorage `cresome.mapAutoHistory`、10 始点 × 2 方向 = 20 通りから履歴を除いた残りから）と重複しない。手動モードは従来どおり始点・向き・視点・再生すべてを手動操作。
...
- 歩行視点の仰角は**経路の斜面に自動追従**する。ルート接線ベクトルの傾斜角（`atan2(dir.y, hypot(dir.x, dir.z))`）を heading と同じ指数平滑（α = 1 − e^(−5dt)）でフォローし、坂を上がると上を向く・下り坂は下を向く・平坦路は水平を見る。ユーザーの W/S・矢印キーによる `ctl.pitch` はその上加算される（±1.2rad クランプ）。PiP 歩行側にも同じ傾斜角が反映される。
...
- 読み込み中は表示インジケータ表示。初期ロード時は canvas を不透明度 0（透過 100%）で開始し、全 GLB 読み込み完了・初期視点確定後に 0.8 秒かけてフェードイン（`BOOT_FADE_DUR`）。
- タブ非表示時は描画停止（復帰時に再開）。
...
#### 第十弾修正（2026-09-11 / 初期ロードフェードイン + 操作パネルの自動/手動モード）
- [x] 1. 地図初期ロード時、不透明度 0（透過 100%）で読み込み → ロード完了・視点確定後に 0.8 秒でフェードイン
  - `MapBackground.jsx`: `renderer.domElement.style.opacity = 0` 初期化、`mapLoaded` フラグを init 完了後に true、`frame()` で `canvasOpacity` を 0→1 へ線形復元（`BOOT_FADE_DUR`）
  - `updateRoute` / `seekTo` の opacity 復元は `mapLoaded` ガード付き（フェード中のルート切替では fadeIn を継続し、急な白色闪烁を回避）
- [x] 2. 操作パネルにモード切替（自動 / 手動）を 1 行目に追加（`MapControls.jsx`）
  - 自動 = 既定（`App.jsx` の `defaultMapConfig.mode = 'auto'`、`cresome.mapConfig` に永続化）
  - 自動モード: 始点・向きのボタンを無効化し、ルート完了（fadeOut → 始点スナップ）のたびに `onRouteLoop` で App 側がランダム再抽選
  - 抽選は直近 10 回履歴（`cresome.mapAutoHistory`、10 始点 × 2 方向 = 20 通りから履歴除外）と重複しない
- [x] 3. `index.css`: `.map-controls-group-row:first-child`（モード段の上辺線なし）、`.map-controls-btn:disabled`（減光）、`.map-controls-btn.active:disabled`（選択中始点のみ減光解除）
- [x] 4. 本 README の「始点選択 / 向き切替 / 速度 / 再生制御」「品質・性能」節を更新
- [x] 5. ビルド検証（npm run build、成功）

#### 第十一弾修正（2026-09-11 / 文字ハロー＋歩行視点の斜面ピッチ追従）
- [x] 1. メインコンテンツ・サイドバーの文字に背景色（白）の影を付与（`index.css`）
  - `.main` / `.sidebar` に `text-shadow` 4 層重ね（0 0 2/4/8/12px var(--bg)）。上下左右均一で約 12px の範囲に広がり、外縁に向けて減衰（重なりの減り）するハロー
  - 地図操作パネル（`.map-bottom`）は `text-shadow: none` で無効化（半透明背景・白文字ボタンへの誤影防止）。カウントダウンオーバーレイは自身の shadow 定義を持つため影響なし
- [x] 2. 歩行視点のカメラ仰角を経路斜面に自動追従（`MapBackground.jsx`）
  - ルート接線の傾斜角 `atan2(dir.y, hypot(dir.x, dir.z))` を heading と同じ指数平滑でフォロー（`slopePitch`）
  - 歩行視点の pitch を `ctl.pitch - 0.04 + …` から `ctl.pitch + slopePitch + …` に変更（上りは上を向く／下りは下を向く／平坦は水平）
  - `snapToRoute` でも `slopePitch` を即座に設定。PiP 歩行視点も同様に反映
- [x] 3. 本 README の「カメラ操作」「品質・性能」節を更新
- [x] 4. ビルド検証（npm run build、成功）
